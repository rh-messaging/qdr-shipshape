...
func CreateSmokeRouterOnlyTopology(builder framework.Builder) *SmokeRouterOnlyTopology {
...
func (s *SmokeRouterOnlyTopology) initializeInteriors() {
	...
	s.IcInteriorEast = defaultInteriorSpec()
	...
	ctx := s.FrameworkSmoke.GetFirstContext()
	s.IcInteriorWest = defaultInteriorSpec()
...
func defaultInteriorSpec() *v1alpha1.InterconnectSpec {
	...
	return &v1alpha1.InterconnectSpec{
		DeploymentPlan: v1alpha1.DeploymentPlanType{
			Size:      2,
			Image:     testcommon.GetEnvProperty(testcommon.PropertyImageQPIDDispatch, "quay.io/interconnectedcloud/qdrouterd:latest"),
			Role:      "interior",
			Placement: "Any",
		},
	}
...
func (s *SmokeRouterOnlyTopology) initializeEdges() {
	ctx := s.FrameworkSmoke.GetFirstContext()
	s.IcEdgeEast1 = defaultEdgeSpec(nameIcInteriorEast, ctx.Namespace)
	s.IcEdgeEast2 = defaultEdgeSpec(nameIcInteriorEast, ctx.Namespace)
	s.IcEdgeWest1 = defaultEdgeSpec(nameIcInteriorWest, ctx.Namespace)
	s.IcEdgeWest2 = defaultEdgeSpec(nameIcInteriorWest, ctx.Namespace)
}
