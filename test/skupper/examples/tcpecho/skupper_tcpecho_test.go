...
import (
	...
	"github.com/rh-messaging/qdr-shipshape/test/skupper/examples/tcpecho"
...
var _ = Describe("skupper-example-tcp-echo", func() {
	...
	It("is able access exposed service from another namespace", func() {
		...
		err = framework.WaitForDeployment(tcpecho.PubCtx.Clients.KubeClient, tcpecho.PubCtx.Namespace, tcpEchoDeployment.GetName(), 1, 10 * time.Second, 1 * time.Minute)
		gomega.Expect(err).To(gomega.BeNil())
